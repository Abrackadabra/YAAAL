...
public class C {
    ...
    ArrayList<int[]> tens = new ArrayList<int[]>();

    void solve() throws IOException {
        tens.add(new int[]{10});
        for (int i = 0; i < 10; i++) {
            int[] last = tens.get(tens.size() - 1);
            tens.add(eFFT.multiply(last, last));
        }
        ...
        for (int i = a.length - 1; i >= 0; i--) {
            ...
            while (i != a.length - 1 && s.length() < 32) s = '0' + s;
            sb.append(s);
        }
        out.println(sb);
        out.println(new FBigInteger(sb.toString(), 2));
