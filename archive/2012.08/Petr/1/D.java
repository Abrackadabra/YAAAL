    ...
    void solve() throws IOException {
        ...
        for (int length = 1; length <= 10; length++) {
            ...
            for (int i = length; i < s.length(); i++) {
                String q = s.substring(i - length, i);
                if (s.charAt(i) == 'A') {
                    if (!good.containsKey(q)) good.put(q, 0);
                    good.put(q, good.get(q) + 1);
                }
                if (!total.containsKey(q)) total.put(q, 0);
                total.put(q, total.get(q) + 1);
            }
            String q = s.substring(s.length() - length, s.length());
            int g = 0;
            if (good.containsKey(q)) g = good.get(q);
            int t = 0;
            if (total.containsKey(q)) t = total.get(q);
            res += 0.1 * (g + 1) / (t + 2);
        }
        out.println(res);
    }
