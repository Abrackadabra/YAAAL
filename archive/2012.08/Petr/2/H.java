...
public class H {
    ...
    void solve() throws IOException {
        int a = nextInt();
        long initOffset = (1L << a) - 2;
        long M = nextLong();
        out.println(gget(M + initOffset));
        //if (map.size() > 1000)
            map.clear();
    }

    double gget(long x) {
        return get(x) * 1d / get(x + 1);
    }
    ...
    long get(long x) {
        ...
        if (offs > d) offs = 2 * d - offs;
        if (offs == d) return 2;

        x = rem + offs;
        long step = Long.lowestOneBit(x);

        long res = get(x - step) + get(x + step);
        map.put(x, res);
        return res;
    }
