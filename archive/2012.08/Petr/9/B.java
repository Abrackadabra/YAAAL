...
public class B {
    ...
    String fileName = "betting";
    boolean testBased = false;
    double solve(int n, int s, double p, int t){
        if(n == 0 || t == 0)
            return 0.0;
        if(s + s >= n){
            return p + (1 - p) * solve(n, 2 * s - n, p, t - 1);
        }
        else
            return p * solve(n, s + s, p, t - 1);
    }
    void solve() throws IOException {
        while (true){
            ...
            if(n ==0)
                return;
            out.println(solve(n,s, p/100.0, t));
